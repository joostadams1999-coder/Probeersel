...
with st.sidebar:
    ...
    with st.form("survey_form"):
        ...
        # Tooltip helpers voor "bezit"
        if bezit:
            st.markdown("<div style='font-size: 12px; color: #666;'>", unsafe_allow_html=True)
            for item in bezit:
                q_id = item.split(":")[0]
                desc = get_quality_description(q_id, st.session_state.get("language", "nl"))
                st.markdown(f"""
                <div style='margin: 5px 0;'>
                    <span title="{desc.replace('"', '&quot;').replace(chr(10), ' ')}" style="cursor: help; border-bottom: 1px dotted #ff7f0e; color: #333;">
                        ✓ {item}
                    </span>
                </div>
                """, unsafe_allow_html=True)
        ...
        if gemist:
            ...
            for item in gemist:
                ...
                st.markdown(f"""
                <div style='margin: 5px 0;'>
                    <span title="{desc.replace('"', '&quot;').replace(chr(10), ' ')}" style="cursor: help; border-bottom: 1px dotted #1f77b4; color: #333;">
                        ✓ {item}
                    </span>
                </div>
                """, unsafe_allow_html=True)
        ...
        # Help tekst voor dropdown opties
        st.markdown("""
        <div style='font-size: 11px; color: #999; margin-top: 10px; font-style: italic;'>
        💡 Hover over de kwaliteiten in de dropdown of gekozen items om de beschrijving te zien
        </div>
        """, unsafe_allow_html=True)
...
if team_responses:
    ...
    # LEGENDA SECTIE
    st.markdown("""
    <div style="border: 2px solid #1f77b4; border-radius: 8px; padding: 15px; margin-bottom: 20px; background-color: #f8f9fa;">
        <h3 style="color: #1f77b4; margin-top: 0; margin-bottom: 15px;">📋 Legenda</h3>
    """, unsafe_allow_html=True)
    ...
    with col_leg1:
        ...
        <div style="display: flex; align-items: center; gap: 10px;">
            <svg width="50" height="50">
                <circle cx="25" cy="25" r="12" fill="rgba(255, 165, 0, 0.8)" stroke="none" />
                <text x="25" y="30" text-anchor="middle" font-size="10" fill="white" font-weight="bold">1.1</text>
            </svg>
            <span><strong>Meest gemist</strong></span>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_leg2:
        ...
        <div style="display: flex; align-items: center; gap: 10px;">
            <svg width="50" height="50">
                <circle cx="25" cy="25" r="12" fill="rgba(255, 165, 0, 0.8)" stroke="black" stroke-width="4" />
                <text x="25" y="30" text-anchor="middle" font-size="10" fill="white" font-weight="bold">1.1</text>
            </svg>
            <span><strong>Meest gebruikt</strong></span>
        </div>
        """, unsafe_allow_html=True)
