...
TRANSLATIONS = {
    "nl": {
        ...
        "legend_title": "Legenda & Uitleg",
        "capability_groups": "Kwaliteitengroepen:",
        "circle_markings": "Cirkelmarkeringen in de grafiek:",
        "most_missed": "Meest gemist",
        "most_missed_desc": "Deze kwaliteit wordt door het meeste aantal teamleden gemist",
        "most_used": "Meest gebruikt",
        "most_used_desc": "Deze kwaliteit wordt door het meeste aantal teamleden gebruikt",
        "circle_size": "Cirkelgrootte:",
        "circle_size_desc": "Hoe groter de cirkel, hoe meer teamleden deze kwaliteit bezitten. Positie: De cirkel staat op de doorsnee positie van alle teamleden die deze kwaliteit hebben."
    },
    "en": {
        "title": "Survey & Dashboard Innovative Capabilities",
        ...
        "legend_title": "Legend & Explanation",
        "capability_groups": "Capability groups:",
        "circle_markings": "Circle markings in the graph:",
        "most_missed": "Most missed",
        "most_missed_desc": "This capability is missed by the most team members",
        "most_used": "Most used",
        "most_used_desc": "This capability is used by the most team members",
        "circle_size": "Circle size:",
        "circle_size_desc": "The larger the circle, the more team members possess this capability. Position: The circle is at the average position of all team members who have this capability."
    }
}
...
def get_group_name(groep, lang="nl"):
    """Get the group name in the correct language"""
    if lang == "nl":
        mapping = {
            "Absorptief": "Absorberen",
            "Adoptief": "Adopteren",
            "Adaptief": "Adapteren"
        }
        return mapping.get(groep, groep)
    elif lang == "en":
        mapping = {
            "Absorptief": "Absorb",
            "Adoptief": "Adopt",
...
with st.sidebar:
    ...
    with st.form("survey_form"):
        ...
        # Invoerveld voor nieuwe teamnaam altijd tonen
        new_team_code = st.text_input(t("team_code_custom")).strip().upper()
...
if team_responses:
    ...
    <div style="border: 2px solid #1f77b4; border-radius: 8px; padding: 20px; margin-bottom: 20px; background-color: #f8f9fa;">
        <h3 style="color: #1f77b4; margin-top: 0; margin-bottom: 20px;">📋 {t('legend_title')}</h3>
    """, unsafe_allow_html=True)
    
    # Legenda - Groepen met expandables
    st.markdown(f"<h4 style='color: #1f77b4; margin-bottom: 10px;'>{t('capability_groups')}</h4>", unsafe_allow_html=True)
    ...
    with col1:
        with st.expander(f"🟠 {t('absorptive_title')}"):
            ...
            lang = st.session_state.get('language', 'nl')
            absorptive_qualities = ['1.1', '1.2', '1.3', '1.4', '1.5']
            quality_list = "".join([f"<li><strong>{q}:</strong> {get_quality_description(q, lang)}</li>" for q in absorptive_qualities])
            st.markdown(f"<ul style='margin-top: 15px; font-size: 13px;'>{quality_list}</ul>", unsafe_allow_html=True)
    ...
    with col2:
        with st.expander(f"🔵 {t('adoptive_title')}"):
            ...
            lang = st.session_state.get('language', 'nl')
            adoptive_qualities = ['2.1', '2.2', '2.3', '2.4', '2.5', '2.6', '2.7']
            quality_list = "".join([f"<li><strong>{q}:</strong> {get_quality_description(q, lang)}</li>" for q in adoptive_qualities])
            st.markdown(f"<ul style='margin-top: 15px; font-size: 13px;'>{quality_list}</ul>", unsafe_allow_html=True)
    ...
    with col3:
        with st.expander(f"🟡 {t('adaptive_title')}"):
            ...
            lang = st.session_state.get('language', 'nl')
            adaptive_qualities = ['3.1', '3.2', '3.3', '3.4', '3.5', '3.6']
            quality_list = "".join([f"<li><strong>{q}:</strong> {get_quality_description(q, lang)}</li>" for q in adaptive_qualities])
            st.markdown(f"<ul style='margin-top: 15px; font-size: 13px;'>{quality_list}</ul>", unsafe_allow_html=True)
    
    # Legenda - Markering voorbeelden met betere contextualisering
    st.markdown(f"<h4 style='color: #1f77b4; margin-top: 20px; margin-bottom: 10px;'>{t('circle_markings')}</h4>", unsafe_allow_html=True)
    lang = st.session_state.get('language', 'nl')
    most_missed_text = TRANSLATIONS[lang]['most_missed']
    most_missed_desc_text = TRANSLATIONS[lang]['most_missed_desc']
    most_used_text = TRANSLATIONS[lang]['most_used']
    most_used_desc_text = TRANSLATIONS[lang]['most_used_desc']
    ...
    with col_leg1:
        ...
        <div style="display: flex; align-items: center; gap: 15px; background-color: #fff9e6; padding: 12px; border-radius: 6px; border-left: 4px solid #ff7f0e;">
            ...
            <div>
                <span><strong>{most_missed_text}</strong></span><br>
                <span style="font-size: 11px; color: #666;">{most_missed_desc_text}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_leg2:
        ...
        <div style="display: flex; align-items: center; gap: 15px; background-color: #e6f2ff; padding: 12px; border-radius: 6px; border-left: 4px solid #1f77b4;">
            <svg width="60" height="60">
                ...
                <text x="30" y="36" text-anchor="middle" font-size="11" fill="black" font-weight="bold">1.1</text>
            </svg>
            <div>
                <span><strong>{most_used_text}</strong></span><br>
                <span style="font-size: 11px; color: #666;">{most_used_desc_text}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div style="background-color: #f0f0f0; padding: 12px; border-radius: 6px; margin-top: 15px; border-left: 4px solid #666;">
        <span style="font-size: 12px; color: #333;">
            <strong>{t('circle_size')}</strong> {t('circle_size_desc')}
        </span>
    </div>
    """, unsafe_allow_html=True)
    ...
    for _, row in df.iterrows():
        ...
        fig1.add_trace(go.Scatter(
            ...
            name=get_group_name(row['groep'], lang), hovertemplate=f"<b>{t('quality')} {row['id']}</b><br>{get_quality_name(row['id'], lang)}<br>{desc}<br>{t('number')}: {row['count']}<extra></extra>"
        ))
        ...
        if len(points) >= 3:
            ...
            try:
                ...
                fig2.add_trace(go.Scatter(x=hull_pts[:,0], y=hull_pts[:,1], fill="toself", fillcolor=groep_kleuren[groep], 
                                         opacity=0.3, line=dict(color=groep_kleuren[groep], width=2), name=get_group_name(groep, lang)))
            except: 
                fig2.add_trace(go.Scatter(x=pts[:,0], y=pts[:,1], mode='lines+markers', line=dict(width=4), name=get_group_name(groep, lang)))
        elif len(points) > 0:
            pts = np.array(points)
            fig2.add_trace(go.Scatter(x=pts[:,0], y=pts[:,1], mode='markers', marker=dict(size=12, color=groep_kleuren[groep]), name=get_group_name(groep, lang)))
